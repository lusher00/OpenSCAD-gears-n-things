...
// Set screw
set_screw_d = 3.50;              // hole diameter
set_screw_z = belt_width / 2;   // 2.0 — centered in belt width
// distance from top face to screw center = 6.6
...
top_flange_h = 6.6 - belt_width / 2;   // 4.6 — extended so top face is 6.6 above screw
...
difference() {
    ...
    // set screw hole (through both walls, centered between lightening holes)
    rotate([0, 0, 180 / lighten_holes])
        translate([0, 0, set_screw_z])
            rotate([0, 90, 0])
                translate([0, 0, -(flange_r + 1)])
                    cylinder(d=set_screw_d, h=2 * (flange_r + 1), $fn=50);
